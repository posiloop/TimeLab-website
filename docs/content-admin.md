...
> **遷移後務必重啟一次網站服務。**
> 前台用 `unstable_cache` 快取查詢結果，且設定為永久快取直到明確失效。
> 若網站在遷移前就已啟動，它會快取住「資料庫是空的」這個結果 ——
> 遷移完成後畫面仍是空白，而且不會自己恢復。
> 開發時是 `rm -rf .next` 後重跑 `bun run dev`，
> 正式環境是 `docker compose restart web`。
> 這只在首次遷移時發生；之後後台每次儲存都會自動讓快取失效。
