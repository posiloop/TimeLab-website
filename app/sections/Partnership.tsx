    ...
    <section id="partnership" className="px-16 max-lg:px-8 max-md:px-4">
      ...
      <ul className="flex flex-col items-center gap-5 pb-6">
        {PARTNERSHIPS.map((item) => (
          <li key={item.id} className="w-[700px] max-w-full">
            {/* 整張卡片都可開啟彈窗，「更多」只是視覺提示。卡片含 h3、p、ul
                等非 phrasing content，包進 <button> 會是無效的 HTML，
                故改以 role="button" 搭配鍵盤處理 */}
            <div
              role="button"
              tabIndex={0}
              ...
              className="group/card flex w-full cursor-pointer flex-col items-center justify-center rounded-[20px] transition-transform duration-[400ms] ease-[cubic-bezier(0.22,1,0.36,1)] hover:scale-[0.96] focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-brand"
            >
              ...
              <div className="flex w-full flex-col items-center justify-center gap-2 rounded-b-[20px] border-x-2 border-b-2 border-brand bg-white p-5">
                ...
                <span className="relative block h-[30px] w-[100px] overflow-hidden rounded-[15px]">
                  {/* hover 時紫色塊由左側圓形展開成整顆膠囊 */}
                  <span
                    aria-hidden
                    className="absolute left-0 top-1/2 h-[30px] w-[30px] -translate-y-1/2 rounded-[15px] bg-brand transition-[width] duration-[400ms] ease-[cubic-bezier(0.22,1,0.36,1)] group-hover/card:w-[100px]"
                  />
                  ...
                  <svg
                    ...
                    fill="none"
                    aria-hidden
                    className="absolute left-[13px] top-1/2 h-4 w-5 -translate-x-[12.5px] -translate-y-1/2 text-brand-mist transition-transform duration-[400ms] ease-[cubic-bezier(0.22,1,0.36,1)] group-hover/card:translate-x-0"
                  >
                    ...
                    <path
                      ...
                      strokeDasharray={1}
                      strokeDashoffset={1}
                      className="transition-[stroke-dashoffset] duration-[400ms] ease-[cubic-bezier(0.22,1,0.36,1)] group-hover/card:[stroke-dashoffset:0]"
                    />
                    <path
                      d="m12 3 5 5-5 5"
                      stroke="currentColor"
                  ...
                  <span className="absolute inset-y-0 left-12 right-4 flex items-center justify-center text-title text-brand-ink transition-colors duration-[400ms] ease-[cubic-bezier(0.22,1,0.36,1)] group-hover/card:text-brand-mist">
                    更 多
                  </span>
                </span>
              </div>
